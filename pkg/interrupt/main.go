...
var (
	...
	// addHandlerChan is used to add an interrupt handler to the list of handlers to be invoked on SIGINT (Ctrl+C)
	// signals.
...
// Listener listens for interrupt signals, registers interrupt callbacks, and responds to custom shutdown signals as
// required
func Listener() {
	invokeCallbacks := func() {
		...
		if Restart {
			...
			if runtime.GOOS != "windows" {
				...
				// var proc *os.Process
				// if proc, e = os.StartProcess(os.Args[0], os.Args[1:], procAttr); E.Chk(e) {
				// }
				var s []string
